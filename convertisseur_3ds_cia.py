...
def convert_to_cia(filename, crc32):
    with open(filename, "rb") as fh:
        ...
        if ret != 0:
            ...
            print("Si ce titre est d'une région différente de celle de votre matériel, vous devez décrypter le fichier CIA à l'aide de Decryp9WIP/Godmode9 avant de l'installer.")
...
def which(cmd):
    ...
    if not path:
        return None
    # Ajoute le dossier 'tools' à la recherche
...
def main_check(filename, remove):
    with open(filename, 'rb') as fh:
        ...
        if get_ncchFlag7(fh) & 0x4:
            print(colorama.Fore.YELLOW + " [NON REQUIS]")
        elif not find_xorpad(titleid, crc32):
            print(colorama.Fore.RED + " [INDISPONIBLE]")
            missing_xorpads.append([filename, crc32])
            return
        else:
            print(colorama.Fore.GREEN + " [DETECTER]")
    if remove:
        os.remove(filename)
...
if __name__ == "__main__":
    ...
    if make_cia is None:
        print(colorama.Fore.RED + "make_cia introuvable.")
        print(colorama.Style.RESET_ALL)
        sys.exit(1)
